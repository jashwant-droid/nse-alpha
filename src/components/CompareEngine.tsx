...
export default function CompareEngine({ selectedStocks, allStocks, onRemoveStock }: CompareEngineProps) {
  if (selectedStocks.length === 0) {
    return (
      <div className="text-center py-12 bg-brand-milk rounded-sm border border-brand-border space-y-2 shadow-sm">
        <TrendingUp size={36} className="text-indigo-500/40 mx-auto animate-bounce" />
        <h3 className="font-semibold text-gray-900 font-display text-sm uppercase tracking-wider">No Stocks Selected for Comparison</h3>
  ...
  if (selectedStocks.length === 1) {
    return (
      <div className="text-center py-12 bg-brand-milk rounded-sm border border-brand-border space-y-2 shadow-sm">
        <Star size={36} className="text-indigo-500/40 mx-auto animate-pulse" />
        <h3 className="font-semibold text-gray-900 font-display text-sm uppercase tracking-wider">Select at Least One More Stock</h3>
        <p className="text-gray-500 text-xs max-w-sm mx-auto leading-relaxed font-mono uppercase">
          You have selected <span className="text-indigo-600 font-bold font-mono">{selectedStocks[0].ticker}</span>. Select another stock to unlock comparative Z-score charts and factor highlights.
        </p>
      </div>
    );
  }
  ...
  const colors = ["#4f46e5", "#10b981", "#d97706", "#db2777"];

  return (
    <div className="space-y-6">
      {/* Side-by-Side Comparison Table */}
      <div className="bg-brand-milk border border-brand-border rounded-sm overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-brand-sand border-b border-brand-border">
                <th className="p-4 font-display text-[10px] font-bold uppercase tracking-widest text-gray-500">
                  Factor Review
                </th>
                {selectedStocks.map((stock, idx) => (
                  <th key={stock.ticker} className="p-4 border-l border-brand-border min-w-[150px]">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <div className="font-display font-semibold text-sm text-indigo-600">{stock.ticker}</div>
                        <div className="text-[10px] text-gray-500 font-mono font-semibold uppercase">{stock.name}</div>
                      </div>
                      <button
                        onClick={() => onRemoveStock(stock.ticker)}
                        className="text-gray-500 hover:text-red-600 text-xs font-mono px-1.5 py-0.5 rounded-sm border border-brand-border hover:border-red-200 hover:bg-red-50 transition-all cursor-pointer"
                      >
                        ✕
                      </button>
                    </div>
            ...
            <tbody className="divide-y divide-brand-border font-mono text-xs text-gray-800">
              {metrics.map((m) => (
                <tr key={m.key} className="hover:bg-brand-sand transition-colors">
                  ...
                  {selectedStocks.map((stock, sIdx) => {
                    ...
                    return (
                      <td 
                        key={stock.ticker} 
                        className={`p-4 border-l border-brand-border text-xs font-mono ${
                          isWinner 
                            ? 'bg-indigo-50/40 text-indigo-700 font-bold border-l-2 border-indigo-600' 
                            : 'text-gray-700'
                        }`}
                      >
                        <div className="flex items-center justify-between gap-1.5">
                          ...
                          {isWinner && (
                            <span 
                              className="text-[9px] bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded-sm font-sans font-semibold flex items-center gap-0.5 border border-indigo-200 uppercase"
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-center">
        {/* Explanation Card */}
        <div className="bg-brand-milk border border-brand-border rounded-sm p-5 space-y-4 lg:col-span-1 shadow-sm">
          <div className="flex items-center gap-2 text-indigo-600">
            <Shield size={18} />
            <h4 className="font-display font-semibold text-xs uppercase tracking-wider text-gray-900">Factor Profile Analysis</h4>
          </div>
          ...
          <div className="space-y-2 text-xs text-gray-500 pt-1 font-mono uppercase tracking-tight text-[11px]">
        ...
        {/* Radar Chart */}
        <div className="bg-brand-milk border border-brand-border rounded-sm p-4 lg:col-span-2 h-[340px] flex items-center justify-center shadow-sm">
          <ResponsiveContainer width="100%" height="100%">
            <RadarChart cx="50%" cy="50%" outerRadius="75%" data={chartData}>
              <PolarGrid stroke="var(--color-brand-border)" opacity={0.6} />
              <PolarAngleAxis 
                dataKey="subject" 
              ...
              />
              <Legend 
                wrapperStyle={{ fontSize: '11px', fontFamily: 'Space Grotesk', color: '#374151' }}
              />
            </RadarChart>
          </ResponsiveContainer>
        </div>
